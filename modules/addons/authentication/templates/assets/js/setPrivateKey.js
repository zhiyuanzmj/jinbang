function setPrivateKey( systemURL, type ) {
	$.ajax({
		type: "GET",
		url: systemURL + "/modules/addons/authentication/genRSA.php?type=" + type,//提交的URL
		dataType:"json",
...
function getalipayKey( systemURL, appid, privateKey,  Publickey) {
	$('#authenticationconfig input[type=text]').val(appid);
	$('#authenticationconfig textarea:first').val(privateKey);
	$('#authenticationconfig textarea:last').val(Publickey);
}
