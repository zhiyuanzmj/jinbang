	...
	<div class="row">
	    ...
	    <div class="col-sm-12">
		    <div class="alert alert-warning" style="margin: 30px 15px">
	...
	<div class="row">
		<div class="col-md-8">
			<div class="form-group col-sm-12">
                ...
                <input type="text" name="cert_name" id="cert_name" class="form-control" value="{if $userinfo->type ne 'Company' or $userinfo->type ne 'Company2'}{$userinfo->cert_name}{/if}" {if $userinfo->validation == 'yes' or $userinfo->validation == 'Pending'}readonly{/if} />
            </div>
            <div class="form-group col-sm-12">
                <label for="cert_no" class="control-label">身份证号</label>
                <input type="text" name="cert_no" id="cert_no" class="form-control" value="{if $userinfo->type ne 'Company' or $userinfo->type ne 'Company2'}{$userinfo->cert_no}{/if}" {if $userinfo->validation == 'yes' or $userinfo->validation == 'Pending'}readonly{/if}  />
            </div>
            
            <div class="form-group col-sm-12 validationdate {if $userinfo->validation != 'yes'}hide{/if}">
                <label for="date" class="control-label">认证时间</label>
                <input type="text" id="validationdate" class="form-control" value="{$userinfo->date}" disabled/>
            </div>
		    <div class="form-group col-sm-12 hide">
			    <div class="checkbox">
		        	<label><input type="checkbox" id="checkbox" value="check" {if $userinfo->validation == 'yes'}disabled{/if}/> 阅读并同意下列条款</label>
			    </div>
		    </div>
		    <div class="form-group col-sm-12">
			    <button type="submit" id="authsubmit" class="btn btn-success" {if $userinfo->validation == 'yes' or $userinfo->validation == 'Pending'}disabled{/if}>
			        <span>
			        	<span>{if $userinfo->validation == 'yes'}已成功进行实名认证{else}提交认证{/if}</span>
					</span>
	...
	(function() {
		$('#authsubmit').click(function(){
			...
			$.ajax({ //调用jQuery的ajax方法
	            ...
	            url: "", //把数据提交
	            data: "cert_name=" + cert_name + "&cert_no=" + cert_no + "&goto=auth",
	            dataType: 'json',
