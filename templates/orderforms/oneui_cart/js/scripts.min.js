...
function updateConfigurableOptions(e, t) {
	jQuery.post("cart.php", "a=cyclechange&ajax=1&i=" + e + "&billingcycle=" + t, function(e) {
...
function recalctotals() {
	...
	var t = jQuery.post("cart.php", "ajax=1&a=confproduct&calctotal=true&" + jQuery("#frmConfigureProduct").serialize());
	t.done(function(t) {
...
function selectDomainPeriodInCart(e, t, n, a) {
	...
	var r = jQuery.post(window.location.pathname, {
		domain: e,
		period: n,
...
function validate_captcha(e) {
	...
	var i = jQuery.post(e.attr("action"), e.serialize() + "&a=validateCaptcha", "json");
	i.done(function(t) {
		...
		if(u[n] !== !0) {
			if(!i && n == h && u[_type] == c && u.name) {
				var Q = t.closest("form"),
					k = 'input[name="' + u.name + '"]';
				k = Q.length ? Q.find(k) : e(k), k.each(function() {
					this !== u && e(this).data(s) && a(e(this), n)
				})
			}
			g ? (u[n] = !0, u[h] && a(t, h, "force")) : (i || (u[n] = !0), y && u[_indeterminate] && a(t, _indeterminate, !1)), d(t, y, n, i)
		}
		u[p] && r(t, _cursor, !0) && m.find("." + l).css(_cursor, "default"), m[_add](b || r(t, n) || ""), m.attr("role") && !g && m.attr("aria-" + (v ? p : h), "true"), m[_remove](C || r(t, j) || "")
	}
	...
	_determinate = "determinate", _indeterminate = "in" + _determinate, _update = "update", _type = "type", _click = "click", _touch = "touchbegin.i touchend.i", _add = "addClass", _remove = "removeClass", _callback = "trigger", _label = "label", _cursor = "cursor", _mobile = /ipad|iphone|ipod|android|blackberry|windows phone|opera mini|silk/i.test(navigator.userAgent), e.fn[s] = function(r, o) {
		...
		var y = e.extend({
			...
			j = y.focusClass || "focus",
			C = y.activeClass || "active",
			b = !!y.labelHover,
			Q = y.labelHoverClass || "hover",
			k = 0 | ("" + y.increaseArea).replace("%", "");
		return g != u && g != c || (d = 'input[type="' + g + '"]'), k < -50 && (k = -50), m(this), f.each(function() {
			var r = e(this);
			i(r);
			var o, d = this,
				f = d.id,
				...
				S = d[_type] == u ? y.checkboxClass || "i" + u : y.radioClass || "i" + c,
				D = e(_label + '[for="' + f + '"]').add(r.closest(_label)),
				M = !!y.aria,
				T = s + "-" + Math.random().toString(36).substr(2, 6),
				P = '<div class="' + S + '" ' + (M ? 'role="' + d[_type] + '" ' : "");
			M && D.each(function() {
				P += 'aria-labelledby="', this.id ? P += this.id : (this.id = T, P += T), P += '"'
			}), P = r.wrap(P + "/>")[_callback]("ifCreated").parent().append(y.insert), o = e('<ins class="' + l + '"/>').css(w).appendTo(P), r.data(s, {
				o: y,
				s: r.attr("style")
			}).css(_), !!y.inheritClass && P[_add](d.className || ""), !!y.inheritID && f && P.attr("id", s + "-" + f), "static" == P.css("position") && P.css("position", "relative"), t(r, !0, _update), D.length && D.on(_click + ".i mouseover.i mouseout.i " + _touch, function(n) {
				var a = n[_type],
					i = e(this);
				if(!d[p]) {
					if(a == _click) {
						...
						t(r, !1, !0)
					} else b && (/ut|nd/.test(a) ? (P[_remove](v), i[_remove](Q)) : (P[_add](v), i[_add](Q)));
					if(!_mobile) return !1;
					n.stopPropagation()
				}
				...
				var t = e[_type],
					i = e.keyCode;
				return t != _click && ("keydown" == t && 32 == i ? (d[_type] == c && d[h] || (d[h] ? a(r, h) : n(r, h)), !1) : void("keyup" == t && d[_type] == c ? !d[h] && n(r, h) : /us|ur/.test(t) && P["blur" == t ? _remove : _add](j)))
			}), o.on(_click + " mousedown mouseup mouseover mouseout " + _touch, function(e) {
				var n = e[_type],
					a = /wn|up/.test(n) ? C : v;
				if(!d[p]) {
					if(n == _click ? t(r, !1, !0) : (/wn|er|in/.test(n) ? P[_add](a) : P[_remove](a + " " + C), D.length && b && a == v && D[/ut|nd/.test(n) ? _remove : _add](Q)), !_mobile) return !1;
					e.stopPropagation()
				}
			})
		})
...
function(e) {
	...
	provider: function() {
		...
		return this.feedbackContainer = function() {
			...
			var t = '<i class="fa fa-fw fa-spinner fa-spin"></i> ';
	...
	dataTable: function() {
		return this.tables = {}, this.register = function() {
			...
			if("undefined" == typeof n.tables[e]) {
				...
				var i = a.data("ordering");
				"undefined" != typeof i && (t.ordering = i);
				var r = a.data("order");
				"undefined" != typeof r && r && (t.order = r);
				var o = a.data("columns");
				"undefined" != typeof o && o && (t.columns = o);
				var d = a.data("auto-width");
				"undefined" != typeof d && (t.bAutoWidth = d), n.tables[e] = n.initTable(a, t)
			} else if("undefined" != typeof t) {
				var s = n.tables[e],
					l = s.init(),
					u = jQuery.extend(l, t);
				s.destroy(), n.tables[e] = n.initTable(a, u)
			}
			return n.tables[e]
			...
			for(var t = this, n = e.target.id, a = WHMCS.ui.confirmation.register(), i = 0, r = a.length; i < r; i++) jQuery(a[i]).on("confirmed.bs.confirmation", function(e) {
				e.preventDefault(), jQuery.post(jQuery(e.target).data("target-url"), {
					token: csrfToken
				}).done(function(e) {
	...
	return this.register = function() {
		...
		jQuery(e).click(function(e) {
			...
			if(t.data("btn-check-toggle")) {
				var a = "Deselect All",
					i = "Select All";
				t.data("label-text-deselect") && (a = t.data("label-text-deselect")), t.data("label-text-select") && (i = t.data("label-text-select")), t.hasClass("toggle-active") ? (n.prop("checked", !1), t.text(a), t.removeClass("toggle-active")) : (n.prop("checked", !0), t.text(i), t.addClass("toggle-active"))
...
jQuery(document).ready(function() {
	...
	jQuery("#order-standard_cart").find("input").iCheck({
		...
		radioClass: "iradio_square-blue",
		increaseArea: "20%"
	}), jQuery("#inputCardNumber").length && (jQuery("#inputCardNumber").payment("formatCardNumber"), jQuery("#inputCardCVV").payment("formatCardCVC"), jQuery("#inputCardStart").payment("formatCardExpiry"), jQuery("#inputCardExpiry").payment("formatCardExpiry"));
	...
	if(jQuery("#frmConfigureProduct").submit(function(e) {
			...
			var t = jQuery("#btnCompleteProductConfig"),
				n = jQuery(t).html();
			jQuery(t).find("i").removeClass("fa-arrow-circle-right").addClass("fa-spinner fa-spin"), jQuery.post("cart.php", "ajax=1&a=confproduct&" + jQuery("#frmConfigureProduct").serialize(), function(e) {
			...
			t.addClass("panel-addon-selected"), t.find('input[type="checkbox"]').iCheck("check"), t.find(".panel-add").html('<i class="fa fa-shopping-cart"></i> ' + localTrans("addedToCartRemove", "Added to Cart (Remove)")), recalctotals()
		}), jQuery(".addon-products").on("ifUnchecked", ".panel-addon input", function(e) {
			var t = jQuery(this).parents(".panel-addon");
			t.removeClass("panel-addon-selected"), t.find('input[type="checkbox"]').iCheck("uncheck"), t.find(".panel-add").html('<i class="fa fa-plus"></i> ' + localTrans("addToCart", "Add to Cart")), recalctotals()
	...
	jQuery("#domain" + jQuery(".domain-selection-options input:checked").val()).show(), jQuery(".domain-selection-options input").on("ifChecked", function(e) {
		...
		if(d.tooltip("hide"), c.length && c.tooltip("hide"), jQuery('input[name="domainoption"]').iCheck("disable"), domainLookupCallCount = 0, t.attr("disabled", "disabled").addClass("disabled"), jQuery(".domain-lookup-result").addClass("hidden"), jQuery("#primaryLookupResult div").hide(), jQuery("#primaryLookupResult").find(".register-price-label").show().end().find(".transfer-price-label").addClass("hidden"), jQuery(".domain-lookup-register-loader").hide(), jQuery(".domain-lookup-transfer-loader").hide(), jQuery(".domain-lookup-other-loader").hide(), "register" == o ? jQuery(".domain-lookup-register-loader").show() : "transfer" == o ? jQuery(".domain-lookup-transfer-loader").show() : jQuery(".domain-lookup-other-loader").show(), jQuery(".domain-lookup-loader").show(), i.find("li").addClass("hidden").end().find(".clone").remove().end(), jQuery("div.panel-footer.more-suggestions").addClass("hidden").find("a").removeClass("hidden").end().find("span.no-more").addClass("hidden"), jQuery(".btn-add-to-cart").removeAttr("disabled").find("span").hide().end().find("span.to-add").show(), r.addClass("hidden").attr("disabled", "disabled"), "register" != o && (a.hide(), jQuery(".suggested-domains").hide()), n.is(":visible") || n.hide().removeClass("hidden").fadeIn(), "register" == o) {
			...
			var h = jQuery.post("cart.php", {
					token: csrfToken,
					a: "checkDomain",
					type: "domain",
					domain: s + l
				}, "json"),
				f = jQuery.post("cart.php", {
					token: csrfToken,
					a: "checkDomain",
					type: "spotlight",
					domain: s + l
				}, "json"),
				p = jQuery.post("cart.php", {
					token: csrfToken,
					a: "checkDomain",
					type: "suggestions",
					domain: s + l
		...
		}), jQuery("#containerNewUserSecurity").hide(), jQuery("#stateselect").attr("required") && jQuery("#stateselect").removeAttr("required").addClass("requiredAttributeRemoved")
		...
		var o = jQuery.post(t.attr("action"), t.serialize() + "&type=domain", "json"),
			d = jQuery.post(t.attr("action"), t.serialize() + "&type=spotlight", "json"),
			s = jQuery.post(t.attr("action"), t.serialize() + "&type=suggestions", "json");
		o.done(function(e) {
			return "object" != typeof e || 0 == e.result.length || e.result.error ? void jQuery(".domain-lookup-primary-loader").hide() : void jQuery.each(e.result, function(e, t) {
				var n = t.pricing,
					a = jQuery("#primaryLookupResult"),
					i = a.find(".domain-available"),
					r = a.find(".domain-price"),
					o = a.find(".domain-contact-support"),
					d = a.find(".domain-unavailable"),
					s = a.find(".domain-invalid");
				if(jQuery(".domain-lookup-primary-loader").hide(), a.find(".btn-add-to-cart").removeClass("checkout"), a.removeClass("hidden").show(), t.isValidDomain) d.hide(), o.hide(), s.hide(), t.isAvailable && "string" != typeof n ? (t.preferredTLDNotAvailable && d.show().find("strong").html(t.originalUnavailableDomain), i.show().find("strong").html(t.domainName), r.show().find("span.price").html(n[Object.keys(n)[0]].register).end().find("button").attr("data-domain", t.idnDomainName)) : (i.hide(), r.hide(), o.hide(), d.show().find("strong").html(t.domainName), "string" == typeof n && "ContactUs" == n && o.show());
				else {
					i.hide(), r.hide(), d.hide(), o.hide();
					var l = s.find("span.domain-length-restrictions");
					l.hide(), t.minLength > 0 && t.maxLength > 0 && (l.find(".min-length").html(t.minLength).end().find(".max-length").html(t.maxLength).end(), l.show()), s.show()
				}
			})
		}).always(function() {
		...
		return r ? (a.tooltip("hide"), jQuery("#captchaContainer").length ? void validate_captcha(t) : (s.tooltip("hide"), l.tooltip("hide"), n.attr("disabled", "disabled").addClass("disabled").find("span").hide().removeClass("hidden").end().find(".loader").show(), void jQuery.post(t.attr("action"), t.serialize(), "json").done(function(e) {
	...
	}), jQuery("#frmConfigureProduct input:visible, #frmConfigureProduct select:visible").first().focus(), jQuery("#frmProductDomain input[type=text]:visible").first().focus(), jQuery("#frmDomainChecker input[type=text]:visible").first().focus(), jQuery("#frmDomainTransfer input[type=text]:visible").first().focus(), jQuery(".tld-filters a").click(function(e) {
		e.preventDefault(), jQuery(this).hasClass("label-success") ? jQuery(this).removeClass("label-success") : jQuery(this).addClass("label-success"), jQuery(".tld-row").removeClass("filtered-row"), jQuery(".tld-filters a.label-success").each(function(e) {
	...
	}), jQuery(".filtered-row:even").removeClass("highlighted"), jQuery(".filtered-row:odd").addClass("highlighted"), jQuery(".promo-cart .btn-add").click(function(e) {
		var t = jQuery(this);
		t.attr("disabled", "disabled").find("span.loading").hide().removeClass("hidden").show().end(), jQuery.post(window.location.pathname, {
			a: "addUpSell",
			product_key: t.data("product-key"),
			token: csrfToken
		}, function(e) {
			return console.log(e.modal), "undefined" != typeof e.modal ? void openModal(e.modal, "", e.modalTitle, "", "", e.modalSubmit, e.modelSubmitId) : void window.location.reload(!0)
		}, "json")
	...
	}), 1 === jQuery("#applyCreditContainer").data("apply-credit") && o.length && (d.iCheck("check"), o.iCheck("check"))
});