    ...
    <table id="tableInvoicesList" class="table table-list hidden">
        <thead>
            <tr>
                ...
                <th class="hidden">{$LANG.invoicesstatus}</th>
                <th style="text-align: center;">{$LANG.invoicesstatus}</th>
        ...
        <tbody>
            {foreach key=num item=invoice from=$invoices}
                <tr onclick="clickableSafeRedirect(event, 'viewinvoice.php?id={$invoice.id}', false)">
                    ...
                    <td class="hidden"><span class="label status status-{$invoice.statusClass}">{$invoice.status}</span></td>
                    <td class="text-center"><span class="status-{$invoice.statusClass}">{$invoice.status}</span></td>
