...
<div class="table-container clearfix">
    <table id="tableEmailsList" class="table table-list hidden">
        ...
        <tbody>
            ...
            <tr onclick="popupWindow('viewemail.php?id={$email.id}', 'emailWin', '720', '450')">
                <td class="text-center"><span class="hidden">{$email.normalisedDate}</span>{$email.date}</td>
                <td>{$email.subject}</td>
                <td class="text-center"><input type="button" class="btn btn-info btn-sm" value="{$LANG.emailviewmessage}" onclick="popupWindow('viewemail.php?id={$email.id}', 'emailWin', '720', '450', 'scrollbars=1,')" /></td>
            </tr>
            {/foreach}
        </tbody>
    </table>
