...
if (!empty($_SESSION['uid'])) {
	...
	add_hook('ClientAreaPageHome', 1, function ($vars) {
		...
		foreach ( $services as $key => $value ) {
			$othProducts[$key]['name'] 			= Capsule::table('tblproducts')->where('id', $value->packageid)->first()->name;
			$othProducts[$key]['id']				= $value->id;
			$othProducts[$key]['domain']			= $value->domain;
			$othProducts[$key]['nextduedate']	= $value->nextduedate;
	    }
	
		return [
			'othProducts' => $othProducts,
		];
	});
}